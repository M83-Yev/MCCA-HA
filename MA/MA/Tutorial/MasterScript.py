...
#
# run = 10
# BA_all = np.zeros((run, len(block_range), len(CC_range)))
# for r in range(run):
#     print(f"starting {r} RUN:")
...
#                 continue
#
#             BA_all[r, j, i] = np.mean(ba)
#
#     end_time = time.time()
#     running_time = end_time - start_time
#     print(f"{r} Run Running time: {running_time:.2f} seconds")
#
# np.save('BA_all.npy', np.array(BA_all))
...
BAs = []
for i in range(1):
    ba, _, _ = LOOCVtool(X_reordered, Y_reordered, method='HA', n_mcca_components=300)
    BAs.append(ba)
# np.save(os.path.join(data_dir, 'results', 'HA_fMRI_Block_test.npy'), BAs)
